...
from MathematicsDepartment.models import MathGroup, MathSchedule, MathHomework
from MathematicsDepartment.tests.test_dependencies import MathematicsDepartmentPaths, \
    MathematicsDepartmentExpectedJsonResponses
from Stats.models import Client, Teacher

mathematics_department_paths = MathematicsDepartmentPaths()
mathematics_department_json_responses = MathematicsDepartmentExpectedJsonResponses()


class MathematicsGroupDataResponseTest(APITestCase):
    def setUp(self):
        ...
        MathGroup.objects.create(
            name='Group 1',
            responsible_teacher=Teacher.objects.get(user=User.objects.get(id=2))
        )

        client = Client.objects.filter(id=1)
        mathematics_group = MathGroup.objects.get(id=1)
        mathematics_group.client.add(*client)

        self.expect_json_response = mathematics_department_json_responses.get_group_data_expected_json_response()

    def test_mathematics_group_data(self):
        response = self.client.get(mathematics_department_paths.build_group_url(), format='json',
...
class MathematicsScheduleDataResponseTest(APITestCase):
    def setUp(self):
        User.objects.create_user('teacher', password='some_random_password')
        User.objects.create_user('client', password='some_random_password')
        ...
        client = Client.objects.filter(id=1)
        math_groups = MathGroup.objects.get(id=1)
        math_groups.client.add(*client)

        MathSchedule.objects.create(
            course_title='ION',
            teacher=User.objects.get(id=1),
            group=MathGroup.objects.get(id=1),
        ...
        self.expect_json_response = mathematics_department_json_responses.get_schedule_data_expected_json_response()

    def test_mathematics_schedule_data(self):
        response = self.client.get(mathematics_department_paths.build_schedule_url(), format='json',
                                   content_filter='application/json')
...
class MathematicsHomeworkDataResponseTest(APITestCase):
    def setUp(self):
        User.objects.create_user('teacher', password='some_random_password')
        User.objects.create_user('client', password='some_random_password')
        ...
        groups = MathGroup.objects.all()
        homeworks = MathHomework.objects.get(id=1)
        homeworks.groups.add(*groups)

        self.expected_json_response = mathematics_department_json_responses.get_homework_data_expected_json_response()

    def test_mathematics_homework_data(self):
        response = self.client.get(mathematics_department_paths.build_homework_url(), format='json',
                                   content_filter='application/json')
